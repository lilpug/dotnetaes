...
namespace DotNetAES.Extensions
{
    public static partial class AESHMAC512DataTableExtensions
    {
        ...
        /// <returns></returns>
        public static DataTable AESHMAC512Encrypt(this DataTable data, AESHMAC512 encryptionEngine, object cryptKey, object authKey)
        {
                    ...
                    foreach (DataColumn col in data.Columns)
                    {
                       if (
                                ...
                                )
                        {
                            ...
                            newRow[col.ColumnName] = encryptionEngine.EncryptToString(dr[col].ToString(), cryptKey,  authKey);
                        }
                        else
                        {
        ...
        /// <returns></returns>
        public static DataTable AESHMAC512EncryptIgnore(this DataTable data, AESHMAC512 encryptionEngine, object cryptKey, object authKey, params string[] ignoreColumns)
        {
            //Checks the table even has values before continuing
            if (data != null && data.Rows.Count > 0)
            {
        ...
        /// <returns></returns>
        public static DataTable AESHMAC512EncryptOnly(this DataTable data, AESHMAC512 encryptionEngine, object cryptKey, object authKey, params string[] onlyColumns)
        {
            //Checks the table even has values before continuing
            if (
